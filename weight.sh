...
for BKGPROCESS in ${BKGPROCESSES}; do
    ...
    for YEAR in ${BKGYEARS}; do
        ...
        for INPUTDIR in $(ls ${BKGDIR} | grep ^${KEY} | grep ${YEAR}); do
            ...
            SCALE1FB=$( echo "scale=10; ${XSECTION}/${SUMMEDWGT}" | bc | awk '{printf "%f", $0}' )
            echo ${BKGPROCESS} ${KEY} ${YEAR} ${LUMINOSITY} ${XSECTION} ${SUMMEDWGT} ${SCALE1FB} >> weightsbkg.txt
            echo "./runNanoLooper --input ${BKGDIR}/${INPUTDIR}/ --output outputs/${BKGPROCESS}_${IFILE}.root --scale1fb ${SCALE1FB} --lumi ${LUMINOSITY} > ~/NanoLooper/logfiles/${BKGPROCESS}_${IFILE}.log 2>&1" >> .jobs.txt
        done
    done
done
...
for SIGPROCESS in ${SIGPROCESSES}; do
    ...
    for YEAR in ${SIGYEARS}; do
        ...
        for INPUTDIR in $(ls ${SIGDIR} | grep ${KEY} | grep ${YEAR}); do
            ...
            echo "./runNanoLooper --input ${SIGDIR}/${INPUTDIR}/ --output outputs/${SIGPROCESS}_${IFILE}.root --scale1fb ${SCALE1FB} --lumi ${LUMINOSITY} > ~/NanoLooper/logfiles/${SIGPROCESS}_${IFILE}.log 2>&1" >> .jobs.txt
        done
    done
done
